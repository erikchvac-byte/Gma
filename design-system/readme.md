...
| **ADR** | `Happy/ADR.md` | 28 architecture decisions — the source of truth for product behavior (age gate, gas math, sort order, stale handling). |
| **UX spines** | `Happy/_bmad-output/planning-artifacts/ux-designs/ux-Happy-2026-06-11/` | `DESIGN.md` + `EXPERIENCE.md`, status **final** — the binding UX contract. **The spines win on conflict with anything in this folder.** |
...
- **Third-party copy is constrained.** Deal descriptions are scraped retailer
  text: rendered as plain text only (markup/emoji stripped), capped near 80
  characters, and suppressed entirely when they match the therapeutic-claim /
  youth-appeal blocklist — never rewritten or embellished.
- **A standing disclaimer footer** sits under the feed footnotes: "Deals are set
  by each retailer and may change without notice — verify in store. Gma's Helper
  is not affiliated with any dispensary and sells nothing. Gas costs are
  estimates based on average fuel prices and rated MPG."
...
color — the "go" of the age-gate confirm, links, the slider thumb, and the
focus ring. (There is no Save button anywhere: vehicle selection applies
immediately, per FR-8.) Everything structural is a
...
**Borders.** Hairline `1px gray-200` is default; `gray-500` (`--border-field`)
for TextField/Select boundaries — the border is the field's only extent
indicator, so it must clear the 3:1 non-text contrast floor; `gray-300`
(`--border-strong`) stays on the secondary button, where the label carries the
meaning; `2px green-700` for focus and emphasis.
...
**Accessibility.** Built in, not bolted on: 44px minimum hit targets (including
the slider — its painted 22px thumb sits inside a 44px touch area), WCAG-AA
contrast on the green, dialog semantics + focus management on the age gate, and
`role="status"`/`role="alert"` on live regions. The vehicle cascade's three
Selects stack one per row so nothing breaks at 320px.
