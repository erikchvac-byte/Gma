    ...
    <section aria-label="Deal feed" style={{ padding: 'var(--space-4)', display: 'grid', gap: 'var(--space-4)' }}>
      ...
      {!loading && (
        <div style={{ display: 'grid', gap: 'var(--space-1)' }}>
          <Notice variant="muted">Last updated <time>{data.meta.lastUpdated}</time></Notice>
          {data.staleCount > 0 && (
            <Notice variant="muted" role="status">{data.staleCount} {data.staleCount === 1 ? 'source' : 'sources'} unavailable</Notice>
          )}
          {/* standing disclaimer — static text, never announced (spine ruling 2026-06-12) */}
          <Notice variant="muted">
            Deals are set by each retailer and may change without notice — verify in store.
            Gma's Helper is not affiliated with any dispensary and sells nothing.
