...
window.GMA_DATA = {
  ...
  // — computed below from the strict stale === true predicate, never hardcoded
  staleCount: 0,
  dispensaries: [
    {
    ...
    {
      id: 'wild-seed', name: 'Wild Seed Wellness', distanceMiles: 31.0, stale: true,
      deals: [{ type: 'daily', description: 'Storewide flower', discountPct: 40, window: 'Active today', countdownMin: null }],
    },
  ],
};
// the same strict predicate drives both omission and the count (ADR-026)
window.GMA_DATA.staleCount = window.GMA_DATA.dispensaries.filter((d) => d.stale === true).length;
