...
>
> **Progress (2026-07-25) — the crawler-reach stack is now COMPLETE and always-on.** Phase 0a-HTML + 0b shipped (**ADR-097**: the homepage renders real deal/store HTML inside `#root`; AgeGate + location onboarding mount behind `inert`/`aria-hidden` overlays so JS crawlers read the feed while humans stay gated). Per-store pages shipped (**ADR-099**: `/store/<id>` with `LocalBusiness` JSON-LD, linked from the homepage crawler-HTML + enumerated in the sitemap — this closes Phase 1a and the JSON-LD half of Phase 1). Per-store honest price drops shipped (**ADR-100**: each SKU priced below its OWN rolling median rendered as prose — "$9 vs usual $18" — NEVER `Offer`/`Product` schema, holding the not-a-seller posture, ADR-066). Availability was solved twice: first a `/healthz` + free UptimeRobot keep-warm (**ADR-098**, monitor #803591198), then **superseded the same day by upgrading Render to the paid Starter plan (ADR-101)** — the instance is now always-on, so a crawl can never hit a cold "service waking up" page. Live-verified 2026-07-25: `robots.txt` allows all + points to the sitemap, `/store/*` serve real prose, 0 `Offer`/`Product` schema and 1 `LocalBusiness` per store page. **Still open (all outward-facing or optional):** submit `sitemap.xml` to Google Search Console + Bing Webmaster Tools (Phase 6); OG/Twitter Card meta tags; human-facing SPA links from the in-app store cards to their `/store/<id>` pages.
>
> **Progress (2026-07-10):** Two server-rendered, crawlable surfaces have shipped and are live on gmaslist.com, both registered before the SPA fallback (the Phase 0a "content in the HTML as delivered by Express" unlock, done page-by-page rather than by injecting into the shell): (1) `/about` + FAQ entity page with Service/FAQPage/WebSite/Organization JSON-LD (**ADR-078**); (2) `/compare` + `/compare/:category` cross-store price-comparison pages with `Dataset` JSON-LD (**ADR-079**) — this realizes the spirit of **Phase 2 (Information Gain)** below, but over the derivation engine's honest derived facts (`disparities.json`/`disparity-rollups.json`), NOT the ingest-time `data.json` stats this doc originally proposed. **Schema note reaffirmed:** both surfaces avoid `Product`/`AggregateOffer` commerce schema per Phase 1's recommendation (`/compare` uses `Dataset`, not a store leaderboard). **Also shipped (2026-07-10, ADR-080):** Phase 1 `robots.txt` + `sitemap.xml` — both as Express routes (`server/routes/sitemapRoute.ts`), the sitemap listing `/`, `/about`, `/compare`, and one `/compare/<category>` URL per live category (enumerated from the same derived rollups the `/compare` index links from). **Phase 0a is now SPLIT (2026-07-11, ADR-082):** the **perf half SHIPPED** — Express injects the `/api/data` payload as an inline `window.__GMA_DATA__` script into the served SPA shell (`server/routes/shellRoute.ts` + `server/utils/buildApiData.ts`); `useDeals` reads it synchronously and skips the boot fetch, killing the JS → `/api/data` → render LCP chain. That is a JS data blob, so it delivers **no** crawler-readable deal HTML — non-JS crawlers (GPTBot/PerplexityBot) still see zero deal content. The **crawler-visible-HTML half remains open and is now 0b-legal-gated**: ADR-082 concluded any human-visible deal markup paints before the JS-mounted AgeGate = the same WAC 314-55-155 question as Phase 0b, so it was deferred to `deferred-work.md` pending the legal call (this supersedes ADR-052's planning view that the inside-`#root` inject could ship without legal review — the implementation folded it behind 0b anyway; do 0a-crawler + 0b together). **Still open:** Phase 0a crawler-HTML half (0b-gated), Phase 0b (AgeGate mounting — legal-gated), the REST of Phase 1 (OG/Twitter meta tags, per-store `LocalBusiness` JSON-LD), Phase 1a per-store routes, Phase 3 (robots policy is settled at "allow all" but not separately reviewed), Phases 5–6 (incl. Search Console sitemap submission).
>
> **Lighthouse baseline (live gmaslist.com, mobile, 2026-07-10):** SEO **100**, accessibility **100**, best-practices **100**; performance **54**, agentic-browsing **43**. Remediation shipped same day (**ADR-081**): `/llms.txt` Express route (Phase 4 below — the SPA shell had been served there, failing the audit), skeleton `minHeight` CLS fix (0.367 → ~0 expected), async Google-Fonts CSS (~870ms off render-blocking). Agentic-browsing should reach ~100 on re-run. **Performance will NOT reach ~100 without Phase 0a feed SSR/deal-data injection** — FCP/LCP (4.1s/4.9s) are bound by the JS → `/api/data` → render chain, which is exactly the Phase 0a blocker this plan already ranks as ~90% of the value. Re-run Lighthouse after deploy to confirm.
>
...
- ✅ **`robots.txt` + `sitemap.xml` — SHIPPED 2026-07-10 (ADR-080).** Served as Express routes (`server/routes/sitemapRoute.ts`), NOT static `client/public` files, so the sitemap can enumerate `/compare/<category>` URLs from the same derived rollups the `/compare` index links from (an entry can never 404). robots allows all crawlers except `/api/`; sitemap lists `/`, `/about`, `/compare` + per-category. **No per-deal URLs** (deals churn hourly → thin rotting pages); per-store URLs wait on Phase 1a.
...
✅ **SHIPPED 2026-07-10 (ADR-081), as an Express route rather than a static file.** Trigger: the first live Lighthouse run (agentic-browsing category) flagged that the SPA fallback was serving the React shell at `/llms.txt` — actively malformed, worse than absent. `buildLlmsTxt`/`llmsTxtRoute` live in `server/routes/sitemapRoute.ts`: llmstxt.org Markdown (H1 + blockquote entity summary reusing `ENTITY_DESCRIPTION`) linking `/`, `/about`, `/compare`, and one `/compare/<category>` per live rollup category — same single-source enumeration as the sitemap. Expectations unchanged: this serves the agentic/dev-tool audience, not Google.
