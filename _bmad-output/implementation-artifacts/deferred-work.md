...
- ~~**Full dark-palette WCAG AA contrast audit**~~ **DONE 2026-06-19** (see DESIGN.md → Colors → Contrast table). All shipped essential text passes AA (text 14.4, all text-muted contexts 6.5–8.3 incl. legal small print + stale line, action button 9.95, semantics 6.1–9.9). Two **visual-polish** follow-ups remain (not AA blockers): (a) **hairline borders** (`--border` vs surfaces ≈ 1.4–2.1:1, below 1.4.11's 3:1) — the gate/deal card edge relies on a faint hairline since `--surface #161F23` is only ~1.1:1 off `--bg #0E1417`; consider raising card/field borders to `--border-strong #3C4D55` for a crisper edge (focus *state* already passes via the teal ring, so this is polish, not a WCAG fail for non-control surfaces). (b) **`--text-faint #5E6E76`** (3.16–3.51:1) is fine while confined to placeholder / struck-price / unrendered specimens, but must be bumped toward AA if ever promoted to readable copy; also drop the now-dead `--status-stale` alias (StaleIndicator uses Notice `muted` = `--text-muted`).
