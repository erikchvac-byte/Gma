...
Code review 2026-07-10 (bmad-code-review, 3 parallel adversarial layers: Blind Hunter [diff-only], Edge Case Hunter [diff + project access], Acceptance Auditor [diff + spec]). Verified state going in: 534/534 server tests, `npm run build` clean. No High-severity defects; all 9 ACs verified conformant and every honesty gate (Gate 2/3, decision-F compile guard covering BOTH prices and potency, FR7 accounting, no-route scope, write-ordering) confirmed by the Auditor. Triage: 0 decision-needed, 3 patch (applied), 1 defer, 5 dismissed.

- [x] [Review][Patch] Bound the per-product walk with `endDate: today` [server/utils/brandPersonas.ts] — `countSpecialDays` previously bounded neither end (unlike sibling `specialEvents.ts`), so a future-dated / clock-skewed observation would be counted as a real special-day and an epoch+recent history could blow the day-walk span. Threaded `today` through `buildBrandPersonas(products, today)` and pass `endDate: today` to the walk (Blind+Edge, Med). Output-identical on current data — re-derived live: 118/39/134/214/826 unchanged.
- [x] [Review][Patch] Trim `displayBrand` [server/utils/brandPersonas.ts] — `pickDisplayBrand` now trims the chosen raw label so a whitespace variant (live: `"Hustler's Ambition "`) can't surface a ragged human-facing label (Auditor+Edge, Low).
- [x] [Review][Patch] Correct the `MIN_OBSERVED_PRODUCT_DAYS` comment [server/utils/brandPersonas.ts] — the old comment claimed it "excludes same-day-onboarded noise," but it counts product-days, not distinct calendar days. Comment now describes it honestly and points to the deferred refinement (Blind+Edge, Med → comment-accuracy).
- [x] [Review][Defer] Distinct-calendar-day floor for classification — a brand carried at ≥10 stores on ONE calendar day clears the product-days floor and earns a confident persona ("always/never") from a single day. The special-frequency metric is defensible (it measures prevalence across the brand's footprint) and this design was deliberate + Erik-approved; a distinct-calendar-day gate would change the validated 118/39/134/214 split and is an Epic-2 / threshold-tuning refinement. Deferred, logged to deferred-work.md.

Dismissed (5): float-equality at the 0.95/0.05 thresholds (inclusive `>=`/`<=`; tested boundary ratios equal the literals; contrived); `special === true` with no coercion (source type is `boolean`, DB reader yields a real boolean); a single malformed `observedAt` zeroing a product's whole contribution (matches the accepted `specialEvents.ts` catch-per-product precedent, no live trigger); AC7 field renamed `brandInsufficientHistoryCount` (the correct fix for a real `DeriveOutcome` name collision, Auditor confirmed not a violation); insufficient-history brands appearing in both `personas[]` and `excluded[]` (exactly per AC6).
...
- 2026-07-10: Code review (bmad-code-review, 3 adversarial layers). No High-severity defects; all 9 ACs + honesty gates verified conformant. 3 patches applied (bound the per-product walk with `endDate: today` matching specialEvents.ts; trim `displayBrand`; correct the MIN-floor comment), 1 deferred (distinct-calendar-day classification floor → deferred-work.md), 5 dismissed. Re-derived live: 118/39/134/214/826 unchanged. 534/534 server tests green, build clean. Status → done.
