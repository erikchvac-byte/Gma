---
name: gmas list
description: Visual identity for gmas list (Happy) — a precise, dark, finance/transit-energy utility that ranks legal cannabis deals by TRUE cost (sticker + gas to get there). "Arcade" direction (ADR-041 — pink brand / amber discount / red urgency; supersedes the Synthwave & Tidewater palettes). Distilled 2026-06-18 from imports/GMAS_LIST_BRIEF.md (canonical) and .decision-log.md. AA contrast UNVERIFIED for the Arcade values — re-audit before final. Token values lifted verbatim from the brief's §2–§6; derivations are tagged [DERIVED] in prose.
status: draft
updated: 2026-06-20
colors:
  # ---- "Arcade" palette (ADR-041). Dark is default & primary. ----
  # AA contrast UNVERIFIED for these values — re-audit before final (see Colors).
  bg: '#050507'             # app background (near-black)
  surface: '#121016'        # cards, rows
  surface-raised: '#1c1922' # chips, inputs, tiles
  border: '#2a2330'         # hairlines, card borders
  border-strong: '#3a3242'  # [DERIVED] brightened hover border (brief §6)
  # ---- Text ----
  text: '#f5f2f7'           # primary
  text-muted: '#a8a0b2'     # secondary / labels
  text-faint: '#6e6676'     # metadata, struck prices, captions
  # ---- Accent (single brand/interactive color) ----
  accent: '#ff3d8b'         # pink — primary accent ON DARK
  accent-hover: '#ff5fa1'   # hover/active state of accent
  accent-soft: 'rgba(255,61,139,0.12)' # low-emphasis pink fill (distance pill / selected row)
  accent-on-light: '#e0276f'# deeper pink for use on LIGHT backgrounds
  # ---- Discount (NEW token, ADR-041 — discount-semantic ONLY) ----
  discount: '#ffc83d'       # amber — discount figures + discount badge + card-top motif line
  # ---- Ink (light surfaces / print) ----
  ink: '#14212A'
  # ---- Semantic (meaning only, never decoration) ----
  success: '#ff3d8b'        # savings / best total (reads as brand pink)
  warning: '#ff5470'        # red — happy-hour urgency
  danger: '#ff3b5c'         # worst total / error
  # ---- Focus ----
  focus-ring: '#ff3d8b'     # [DERIVED] = accent; visible ring on dark
  scrim: 'rgba(2,2,4,0.7)'  # [DERIVED] flat sheet scrim, no blur
typography:
  display:
  ...
  wordmark:
    ...
    note: 'lowercase "gmas list" + pink set-dot. Never capitalize, never add a pictorial icon.'
  body:
...
The system is **dark by default and primary** (`--bg #050507`). Light surfaces are rare; when they occur the accent swaps to `{colors.accent-on-light}` so the pink stays legible.

**Wordmark (locked):** lowercase `gmas list` in {typography.wordmark} (Space Grotesk Medium, ls −0.01em, closed up tight), followed by a **pink square "set-dot"** — the brand signature. The dot ≈ 0.21× cap height, `{rounded.dot}` (2px), sits near the baseline just right of the final "t". On dark: text `{colors.text}`, dot `{colors.accent}`. On light: text `{colors.ink}`, dot `{colors.accent-on-light}`. Monochrome: dot matches text. **Never** capitalize, never add a pictorial icon, never a cannabis leaf. Minimum width 96px; clear space all sides = cap height; below 96px use the app-icon mark. The in-product brand text is **always** `gmas list` lowercase (the spoken "grandma's" is never leaned on, never written out).

**App icon / favicon — "g + dot":** lowercase `g` in Space Grotesk Bold + the pink set-dot, centered in a rounded-square tile (corner ≈ 24% of size). Standard: `{colors.text}` "g" + `{colors.accent}` dot on `{colors.bg}` tile. Alt: `{colors.bg}` "g" + `{colors.bg}` dot on `{colors.accent}` tile. Ship 16/32/56/96 + 180 (apple-touch) + 512.

## Colors

"Arcade" (ADR-041) — a near-black base with a high-contrast **three-color** system. Tokens are CSS custom properties; dark is the default and primary mode. Color is used sparingly and always means something. Supersedes the Synthwave/Tidewater palettes.

- **Surfaces** layer by lightness: `{colors.bg}` app background (near-black), `{colors.surface}` cards and rows, `{colors.surface-raised}` chips/inputs/tiles, `{colors.border}` subtle hairlines/dividers, `{colors.border-strong}` the crisper card & field edge. Depth comes from these steps and from borders — not from shadow (see Elevation & Depth).
...
- **Accent (pink)** `{colors.accent}` `#ff3d8b` is the single **brand/interactive** color. Use it sparingly: primary buttons, the active chip, the focus ring, links, the logo set-dot, the distance pill, the gas line. Hover/active brightens to `{colors.accent-hover}`. `{colors.accent-soft}` is a low-emphasis pink fill (distance pill / selected row). On the rare light surface, swap to `{colors.accent-on-light}` `#e0276f`. **Text on an accent fill is always `{colors.bg}` `#050507`, never white** (brief §2/§8).
- **Discount (amber)** `{colors.discount}` `#ffc83d` is **discount-semantic ONLY** — the per-deal discount figure and the discount badge — plus the one sanctioned decorative use, the card-top motif line. Never a general accent. Magnitude stays a single hue with a font-weight step only, never a second color (ADR-037/040/041 invariant).
- **Semantic** colors mean state, never decoration: `{colors.warning}` `#ff5470` (red) = happy-hour **urgency**, `{colors.danger}` worst total / error, `{colors.success}` savings / best total (reads as the brand pink).

> **Three signals, kept legible:** pink = interactive, amber = discount, red = urgency. Do not introduce a fourth accent.

**Contrast — AA contrast UNVERIFIED — re-audit before final.** The prior audit (2026-06-19) validated the *Tidewater* pink palette; the Arcade swap (ADR-041) changes every load-bearing pair — amber-on-near-black, pink-on-near-black, red-on-near-black, and the new near-black surface ladder — so **no AA claim holds until these are re-measured.** This section must not state or imply AA compliance until a fresh sRGB-ratio audit of the Arcade values is run; the old per-pair ratio table was removed as no longer applicable. Carry-over guardrails that are palette-independent still apply: `{colors.text-faint}` stays confined to incidental/non-essential text (WCAG 1.4.3) and must never carry essential readable content (the stale-source line uses `{colors.text-muted}`); card/divider borders are decorative boundaries, with the required focus *state* carried by the pink focus ring. Re-audit task logged in `deferred-work.md`.
...
**Focus is its own depth cue:** a 2px `{colors.focus-ring}` (pink) outline at 2px offset — always visible, never removed without replacement. On dark this pink ring reads clearly against every surface.
...
| Do | Don't |
|---|---|
| Brand accent pink `{colors.accent}`; discount amber `{colors.discount}` (discount-only); urgency red `{colors.warning}` | Introduce a fourth accent, use pink for discount, or use amber as a general accent |
...
| `{colors.bg}` text on the pink accent | White text on the pink accent (brief §8) |
| Cards = 1px `{colors.border-strong}` edge on `{colors.surface}`, flat; best = 1.5px accent | Drop shadows on cards (only the sheet floats) |
| Semantic color for meaning only (savings / caution / worst-cost) | Color-code categories or use semantic color as decoration |
| Lowercase `gmas list` wordmark + pink set-dot | Capitalize the wordmark, or add a pictorial/leaf icon to it |
| Abstract pink-dot brand; line icons that support data | Cannabis leaves, buds, smoke, "420", emoji, mascots/cartoons |
