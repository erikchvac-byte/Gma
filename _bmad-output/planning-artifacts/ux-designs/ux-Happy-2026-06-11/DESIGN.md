...
- **Surfaces** layer by lightness: `{colors.bg}` app background, `{colors.surface}` cards and rows, `{colors.surface-raised}` chips/inputs/tiles, `{colors.border}` subtle hairlines/dividers, `{colors.border-strong}` the crisper card & field edge. Depth comes from these steps and from borders — not from shadow (see Elevation & Depth).
